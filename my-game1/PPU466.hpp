...
struct PPU466 {
  ...
  // pass the size of the current framebuffer in pixels so it knows how to scale
  // itself
  ...
  enum : uint32_t { ScreenWidth = 256, ScreenHeight = 240 };

  // Background Color:
  // The PPU clears the screen to the background color before other drawing
  // takes place.
  ...
  enum : uint32_t { BackgroundWidth = 64, BackgroundHeight = 60 };
  ...
  // tile 0-3 is reserved for background rendering
  ...
  struct Sprite {
    ...
    uint8_t y = 240;   // y position. 0 is the bottom edge of the screen. >= 240
                       // is off-screen
  ...
  //
  // The observant among you will notice that you can't draw a sprite moving off
  // the left
  //  or bottom edges of the screen. Yep! This is [similar to] a limitation of
  //  the NES PPU!

  // Sprites:
  // The PPU always draws exactly 64 sprites:
  //  any sprites you don't want to use should be moved off the screen (y >=
  //  240)
